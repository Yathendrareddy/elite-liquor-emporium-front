...
// UI Components (Assuming these are wrapped Tailwind components)
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { useIsMobile } from "@/hooks/use-mobile";
import {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-white shadow-md h-16 md:h-20">
      ...
      <div className="container mx-auto px-4 h-full">
        <div className="flex items-center justify-between h-full">
          {/* Logo */}
          <Link to="/" className="text-2xl md:text-3xl font-serif font-bold">
            <span className="text-accent">LIQUOR TAP</span>
          </Link>
      ...
      {isMobileMenuOpen && (
        <div className="absolute top-full left-0 w-full bg-white z-50 px-4 py-6 animate-slide-down shadow-lg">
          <form onSubmit={handleSearchSubmit} className="relative mb-6">
            <Input
              type="search"
              placeholder="Search premium spirits..."
          ...
          <nav className="flex flex-col space-y-5">
            {navLinks.map((link) => (
              <Link
                ...
                to={link.path}
                onClick={() => setIsMobileMenuOpen(false)}
                className={`text-lg py-1 hover:text-accent transition-colors ${
                  isActive(link.path) ? 'text-accent font-medium' : ''
                }`}
              >
                {link.name}
